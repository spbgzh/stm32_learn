...
#define LCD_Buff_Size 1920
#define LCD_Total_Size 240*280*2
static unsigned char lcd_buff[LCD_Buff_Size];
/**********SLEEP IN***************/
...
static unsigned char disp_p = 0;
unsigned char SendBuff8[240*10*2];

void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi)
{
}

void Test_DMA(unsigned int color){
	unsigned int i = 0;
	unsigned int j; 
	unsigned char data[2]={0};
	data[0] = color >> 8;
	data[1] = color;
	for(j=0;j<240*20/2;j++)
	{
		SendBuff8[j*2] = data[0];
		SendBuff8[j*2+1] = data[1];
	}
	LCD_Address_Set(0,0,240-1,280-1);
	LCD_CS_Clr();
	
	while(i<=280/10){
		i++;
		HAL_SPI_Transmit_DMA(&hspi1, SendBuff8, 240*20);
...
{          
	...
	data[0] = color >> 8;
	data[1] = color;
	LCD_Address_Set(0,0,240-1,280-1);//������ʾ��Χ
	for(j=0;j<LCD_Buff_Size/2;j++)
	{
		lcd_buff[j*2] = data[0];
		lcd_buff[j*2+1] = data[1];
		
	}
	for(i=0;i<LCD_Total_Size/LCD_Buff_Size;i++)
	{													   	 	
		LCD_Writ_Bus(lcd_buff,LCD_Buff_Size);
	} 				    
} 
